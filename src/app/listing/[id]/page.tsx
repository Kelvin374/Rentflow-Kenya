...
import { fetchPropertyById, fetchNearbyProperties, saveProperty, unsaveProperty, fetchSavedPropertyIds } from '@/lib/supabase-api';
  ...
  useEffect(() => {
    ...
    fetchPropertyById(id).then((p) => {
      ...
      if (p?.id) {
        ...
        if (user) {
          fetchSavedPropertyIds(user.id).then((ids) => {
      ...
      <main className="max-w-[1280px] mx-auto px-6 mt-6">
        ...
        <header className="mb-6 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={async () => {
                if (!user) { showToast('Please log in to save properties', 'info'); return; }
                if (!property) return;
                if (isSaved) {
                  const { error } = await unsaveProperty(user.id, String(property.id));
                  if (!error) { setIsSaved(false); showToast('Removed from saved', 'success'); }
                } else {
                  const { error } = await saveProperty(user.id, String(property.id));
                  if (!error) { setIsSaved(true); showToast('Property saved!', 'success'); }
                }
              }}
              className={`flex items-center gap-2 px-4 py-2 hover:bg-surface-container-low rounded-lg transition-colors border ${isSaved ? 'border-error/30 bg-error/5' : 'border-outline-variant'}`}
            >
              <span
                className="material-symbols-outlined text-[20px]"
                style={isSaved ? { fontVariationSettings: "'FILL' 1", color: 'var(--md-sys-color-error)' } : {}}
              >favorite</span>
              <span className={`text-sm font-semibold ${isSaved ? 'text-error' : ''}`}>{isSaved ? 'Saved' : 'Save'}</span>
            </button>
          </div>
        </header>
            ...
            <div className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant shadow-lg sticky top-24">
              ...
              <div className="space-y-2 mb-6">
                {property.contactPhone && (
                  <a
                    href={`tel:${property.contactPhone}`}
                    className="w-full bg-primary text-on-primary font-bold py-3.5 rounded-xl hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                  >
                ...
                {property.contactPhone && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full bg-[#25D366] text-white font-bold py-3.5 rounded-xl hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                  >
                ...
                {property.contactEmail && (
                  <a
                    href={`mailto:${property.contactEmail}`}
                    className="w-full bg-surface-container-low text-on-surface font-bold py-3.5 rounded-xl hover:bg-surface-container-high transition-all flex items-center justify-center gap-2 border border-outline-variant"
                  >
                ...
                {!property.contactPhone && !property.contactEmail && (
                  <button
                    onClick={() => showToast('No contact information available for this property. Please try again later.', 'info')}
              ...
              <p className="text-center text-xs text-on-surface-variant mt-3">
                {(property.contactPhone || property.contactEmail) ? 'No payment required to book' : 'Contact the property manager to schedule'}
              </p>
            </div>
          </div>
        </div>
